...
PROJECT_ROOT = load_environment(PROJECT_ROOT)
...
def process_ask(request):
    try:
        ...
        if topic and (result.get("grounding") or {}).get("extractive_fallback"):
            if not policy.direct_topic_match(normalized, retrieved.get("evidence", [])):
                return QAResponse(answer="Chưa thể xác nhận nguồn phù hợp với chủ đề này khi LLM không sẵn sàng. Vui lòng thử lại sau hoặc làm rõ tên chủ đề.",
                                  status="error", citations=[],
                                  grounding={"status": "review_unavailable", "no_answer": True},
