...
class PolicyTests(unittest.TestCase):
    ...
    def test_selected_topic_preserves_document_mindmap_intent(self):
        ...
        self.assertTrue(self.generator.calls[0][0]["LAST_ROUND"])
    ...
    def test_irrelevant_extractive_fallback_is_not_an_answer_or_proof_of_absence(self):
        evidence = self.retrieval.retrieve("Transformer")
        self.generator.available = False
        with patch.object(self.retrieval, "retrieve", return_value=evidence):
            response = self.client.post("/api/ask", json={"question": "Bitcoin là gì?"})
        self.assertEqual(response.status_code, 200, response.text)
        result = response.json()
        self.assertEqual(result["status"], "error")
        self.assertEqual(result["grounding"]["status"], "review_unavailable")
        self.assertEqual(result["citations"], [])
