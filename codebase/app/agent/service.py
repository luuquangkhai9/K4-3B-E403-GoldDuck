...
class ChatAgent:
        ...
        if plan.task == "study_materials":
            if not topic:
                ...
                base = self._study(question, topic, search_query, effective_scope, base, reference_ids,
                                   single_review=bool(clarification_topic))
    ...
    def _study(self, question, topic, query, scope, base, reference_ids, *, single_review=False):
        ...
        for round_number in range(1 if reference_ids is not None or single_review else 2):
            trace["retrieval_rounds"] += 1
            ...
            if not self.generator.available:
                ...
                break
            try:
                review = self._review(tools, question, topic, last_round=bool(round_number) or reference_ids is not None or single_review)
                ...
                break
            if round_number or reference_ids is not None or single_review or not review.missing_queries:
                break
