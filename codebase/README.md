...
Prototype chạy thật nằm trong `codebase/`. Từ thư mục gốc repository, chạy `cd codebase` trước các lệnh dưới đây. Hướng dẫn nộp bài nằm trong README gốc; SPEC đã chốt nằm ở `../spec.md`.

Checkout đã có `.env` và `data/` ở gốc sẽ tự động dùng lại chúng khi `codebase/.env` chưa tồn tại. Không cần sao chép key hoặc chạy lại Vision. Nếu đang dùng checkout đó, bỏ qua bước tạo `.env` mới và dùng Python của `../.venv/Scripts/python.exe`, hoặc kích hoạt môi trường đã có. Nếu tạo `codebase/.env`, đường dẫn tương đối PDF/index sẽ được tính từ `codebase/`.
...
if (!(Test-Path .env) -and !(Test-Path ../.env)) { Copy-Item .env.example .env }
...
UI hiện tại gọi backend thật, không lấy câu trả lời từ mock. `../mock/duck-v2.html` chỉ là bản mock checkpoint cũ. Các kiểm thử dùng provider fixture để không tốn API; `scripts/verify_live.py` và `scripts/verify_agent.py` gọi dịch vụ cấu hình thật. Kết quả LLM chưa thẩm định có trạng thái `partial` và vai trò `candidate`; danh sách này chưa phải lộ trình học đã được xác nhận. Credit LLM của tài khoản hiện tại đã hết tại lần kiểm tra tích hợp; cần bổ sung credit để thẩm định trực tuyến.

Câu hỏi thiếu chủ đề sẽ có lựa chọn từ tiêu đề tài liệu đúng phạm vi Day và ô nhập chủ đề khác. Khi làm rõ, hệ thống giữ câu hỏi và ý định ban đầu. Khi thiếu bằng chứng, gợi ý xem tiếp có nguồn được trình bày riêng với câu trả lời. Policy offline không dùng độ dài câu hỏi làm tiêu chí mơ hồ và không gán cứng tên chủ đề.
