...
Đã hợp nhất lịch sử local `main` và `origin/main` (hai lịch sử không có common ancestor), bảo toàn bằng các nhánh `backup/main-before-integration` và `backup/quangkhai-before-main`, sau đó merge `quangkhai` vào `main`.

Giữ nguyên template README của main, điền tên/phân công có thật từ SPEC; để trống mã học viên chưa được cung cấp. Giữ bản SPEC CP4 của main, chỉ đổi tên thành `spec.md`, không sửa quality bar hoặc số đo cũ. Giữ hướng dẫn, examples, tracks, further-reading và mock checkpoint của main. Prototype, hướng dẫn vận hành, cấu hình mẫu, dependency và script chuyển vào `codebase/`.

Resolver đường dẫn hỗ trợ cả checkout mới dùng `codebase/.env`, `codebase/data/` và checkout hiện có dùng `.env`, `data/` ở gốc. Không đổi key/model, không chạy lại Vision hoặc sửa chỉ mục. Kiểm tra metadata thật: 33 PDF, 2.604 slide, 10 Day; 529 slide Vision thành công, 2.075 slide được đánh dấu skipped theo cấu hình ingestion hiện có.

Loại data pack, chỉ mục, log prompt/response, debug log và thư viện `.demo-deps` khỏi tracking ở tip mới; giữ file local và bổ sung ignore. Các file đã từng commit vẫn còn trong lịch sử Git; merge này không phải thao tác purge lịch sử. Chưa push remote.

Kiểm tra sau chuyển đổi:

- 172 kiểm thử Python: qua; 19 kiểm tra UI: qua.
- 20 ca HTTP thật: qua về schema, routing, phạm vi Day, metadata, nguyên văn nguồn và mở trang PDF. Kết quả tối thiểu ở `../eval/http_results.json`; không công nhận đây là 20/20 đánh giá ngữ nghĩa.
- Chrome thật: mindmap tài liệu và mở nguồn từng file, hỏi lại rồi chọn chủ đề vẫn giữ câu hỏi/Day/intent, abstention có gợi ý thật; không có lỗi JavaScript.
- Làm rõ chỉ dùng một lượt thẩm định tài liệu, không truy hồi bổ sung bằng LLM. Khi LLM hỏng, QA không dùng trích xuất từ đoạn không nhắc trực tiếp chủ đề làm câu trả lời; trả trạng thái chưa thể thẩm định, không suy ra thiếu dữ liệu.
- `update_metadata.py` từ gốc repository xác nhận chỉ mục đã đúng và `updated: false`.
- Checkout sạch xuất bằng `git archive`, không có `.env`, dữ liệu hay `.demo-deps`: 172 kiểm thử Python và 19 kiểm tra UI qua.
- Khởi động bằng `codebase/scripts/run_demo.py` từ gốc repository: health OK; chạy lại đủ 20 ca HTTP qua trên cổng 8000. Demo được để chạy tại `http://127.0.0.1:8000`.

Tập 20 ca tích hợp được bổ sung mới; bộ gốc tương ứng số 5/20 trong SPEC không có trong các nhánh, nên không so sánh hai số đó. Chưa có slide demo PDF, video demo, log validation năm người hoặc reflection cá nhân trong nhánh nguồn; không tự tạo bằng chứng thay cho nhóm. LLM hết credit vẫn là giới hạn ngoài mã. Cần bổ sung credit và đánh giá ngữ nghĩa để xác nhận đạt quality bar đã khóa.

Lệnh chạy từ gốc, dùng môi trường đã có:

```powershell
.\.venv\Scripts\python.exe .\codebase\scripts\run_demo.py
```
